...
class Jog(object):
    ...
    """
    Les modes implémentés pour l'instant sont :
    COMMAND_UNDEFINED : Par défaut. Permet de tester les asservissements.
    ...
    def target(self, p):
        if self.command_type == Jog.COMMAND_MANUAL:
            self._target = [p[0], (p[1] + pi / 180 * self.theta) % 2 * pi]  # TODO : à vérifier
        else:
            self._target = p
        self.target_reset = False
    ...
    def manual_dir(self):
        ori = (pi * self._target[1] / 180.0 + self.theta) % (2 * pi)  # TODO : à vérifier
        return [self._target[0] * cos(ori), self._target[0] * sin(ori)]
    ...
    def order(self, last_theta):
        ...
        if self._target is not None:
            ...
            if abs(delta_ori) > pi / 8:  # Requiert de tourner avant de commencer à avancer
