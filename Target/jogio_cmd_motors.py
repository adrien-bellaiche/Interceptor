...
from jogio import fpgaRead, fpgaWrite
...
def motorsSetSpeed(sLeft, sRight):
    """
    set speed for both motors :
    sLeft : speed of letf motor
    sRight : speed of right motor
    """
    motorLeftSetSpeed (sLeft)
    motorRightSetSpeed (sRight)

def motorsSetDirection(dirLeft, dirRight):
    """
    set direction for both motors :
    dirLeft : if >= 0 forwards, if < 0 backwards
    dirRight : if >= 0 forwards, if < 0 backwards
    warning : speed is set to 0 before changing direction
    """
    if (dirLeft<0):
        motorLeftBackward ()
    else: 
